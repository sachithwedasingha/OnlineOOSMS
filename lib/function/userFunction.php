...
public function Authentication($userName,$pwd){
                ...
                if($rec['login_status'] == 1){
                    //lets check the user role
                        ...
                        switch($logRole){
                            ...
                            {
                              header('location:../view/userOption.php');
                            }
                            else
                            {
                            ...
                            case "Store Keeper":
                                ...
                                case "Manager":
                                  ...
                                  case "Technical Officer":
                                    ...
                                    if(!isset($_COOKIE[$location]))
                                    {
                                      header('location:../view/technicalofficer.php');
                                    }
                                    else
                                    {
                                      header($_COOKIE[$location]);
    ...
    function service_count(){
      ...
      $sqlSelect = "SELECT * FROM order_service_tbl 
      INNER JOIN service_status_tbl 
      ON order_service_tbl.status_Id = service_status_tbl.status_Id  
      WHERE  deliverd = 0 AND order_service_tbl.d_status = 0 ORDER BY os_Id DESC;";
       //lets check the errors 
        if($this->dbResult->error){
        echo($this->dbResult->error);
        exit;
